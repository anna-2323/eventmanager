...
Session* session_create(User* u) {
    for (int i = 0; i < SESSION_MAX; i++) {
        if (sessions[i].token[0] == '\0') {
            ...
            sessions[i].user_id = u->id;
            strncpy_s(sessions[i].email, sizeof(sessions[i].email) - 1, u->email, sizeof(sessions[i].email) - 1);
            strncpy_s(sessions[i].phone, sizeof(sessions[i].phone) - 1, u->phone, sizeof(sessions[i].phone) - 1);
            strncpy_s(sessions[i].first_name, sizeof(sessions[i].first_name) - 1, u->first_name, sizeof(sessions[i].first_name) - 1);
            strncpy_s(sessions[i].last_name, sizeof(sessions[i].last_name) - 1, u->last_name , sizeof(sessions[i].last_name) - 1);
            sessions[i].role = u->role;
